...
def read_img(idx, base_path, set_name, version=None):
    ...
    im = cv2.imread(img_rgb_path)
    im = im[:, :, ::-1]
    return im


def img_path(idx, base_path, set_name, version=None):
    if version is None:
        version = sample_version.gs
...
# modified
def read_org(idx, base_path, version=None):
    if version is None:
        version = sample_version.gs
    mask_path = os.path.join(base_path, 'training', 'rgb',
                             '%08d.jpg' % sample_version.map_id(idx, version))
    _assert_exist(mask_path)
    im = cv2.imread(mask_path)
    im = im[:, :, ::-1]
